...
export interface RenderWindowedDiffOptions<LAnnotation = undefined> {
  /** Full old-file blob. */
 ...
 * anything on expand.
 */
export async function renderWindowedDiffHTML<LAnnotation = undefined>({
  oldFile,
  newFile,
  window,
  ...
  options,
  annotations,
}: RenderWindowedDiffOptions<LAnnotation>): Promise<string> {
...
function getWindowedHunksRendererOptions<LAnnotation>(
